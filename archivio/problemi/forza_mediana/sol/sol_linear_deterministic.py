...
def select(v, n, rank):
    ...
    if n == 3:
        ...
        if v[2] == mid:
            return select(v, 2, rank)
        if v[0] == mid:
            return select(v[1:], 2, rank)
        v[1] = v[2]
        return select(v, 2, rank)

    v_of_meds_size = n // 3
    v_of_meds = [0] * v_of_meds_size
    for j in range(v_of_meds_size):
        v_of_meds[j] = get_med_of_3(v[3*j], v[3*j + 1], v[3*j + 2])

    pivot = select(v_of_meds, v_of_meds_size, v_of_meds_size // 2)
    pivot_pos = 0
    while pivot_pos < n:
        if v[pivot_pos] == pivot:
            break
        pivot_pos += 1
    v[pivot_pos] = v[n - 1]
    v[n - 1] = pivot
    ...
    if left == rank:
        return pivot
    if left > rank:
        return select(v, left, rank)

    return select(v[left + 1:], n - left - 1, rank - left - 1)
        ...
        if n == 1:
            ...
            for i in range(n):
                if i != extA and i != extB:
                    candidates.append(i)
            print(select(candidates, n-2, n//2 - 1), flush=True)
